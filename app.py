...
# st.set_page_config(layout="wide", page_title="SOC & SOH Prediction")
# model_soc = tf.keras.models.load_model('gru_soc_model.h5')
# scaler_params_soc = np.load('scaler_params_soc.npy', allow_pickle=True)

# def SOC_pred(data):
#     min_val, scale_val = scaler_params_soc
#     scaler = MinMaxScaler()
#     scaler.min_ = min_val
#     scaler.scale_ = scale_val

#     # Normalize the input data
#     selected_rows_scaled = scaler.transform(data)

#     # Reshape the input data to match the model's input shape
#     time_steps = 100  # Should match the time_steps used during training
#     n_features = selected_rows_scaled.shape[1]  # Should match the number of features in your input data
#     input_data = []
#     for i in range(len(selected_rows_scaled) - time_steps + 1):
#         input_data.append(selected_rows_scaled[i : i + time_steps])
...
#     # Perform prediction
#     predicted_soc_scaled = model.predict(input_data)
...
#     # Print or use the predicted SOC values
#     print("Predicted SOC (Original Scale) for the input data:")
#     print(predicted_soc_original_scale)
...
# st.write("## GRU Prediction of SOC & SOH")
# st.write(
...
# st.sidebar.write("## Upload SOC & SOH for Prediction")
...
# my_upload_soc = st.sidebar.file_uploader("Upload an Excel File for SOC", type=["xlsx"])
# my_upload_soh = st.sidebar.file_uploader("Upload an Excel File for SOH", type=["xlsx"])


# if my_upload_soc is not None:
#     soc_data = pd.read_excel(my_upload_soc)
#     SOC_pred(soc_data)
#     st.write(soc_data)
#     print(soc_data)
...
# st.set_page_config(layout="wide", page_title="SOC Prediction")
...
#     min_val, scale_val = scaler_params_soh
#     scaler = MinMaxScaler()
#     scaler.min_ = min_val
#     scaler.scale_ = scale_val
...
#     # Load the trained GRU model
#     model = tf.keras.models.load_model('gru_soh_model.h5')
...
#     # Print or use the predicted SOC values
#     print("Predicted SOH (Original Scale) for the input data:")
#     print(predicted_soh_original_scale)    


# # Create a title for the app
# st.header("SOC Prediction")

# # Create a sidebar
# sidebar = st.sidebar

# # Create a file uploader widget in the sidebar
# soc_upload = sidebar.file_uploader("Upload an Excel File for SOC", type=["xlsx"])
# soh_upload = sidebar.file_uploader("Upload an Excel File for SOH", type=["xlsx"])

# # If a file is uploaded, read it into a Pandas DataFrame and perform SOC prediction
...
#     st.write("#### Input SOC Data DataFrame")
#     st.dataframe(soc_data)

#     # Display the predicted SOC
#     st.write("Predicted SOC:")
#     st.write("Predicted SOC ", "{:.2f}".format(predicted_soc[0][0]))


# # Otherwise, display a message to the user
# else:
#     st.write("Please upload an Excel file to predict SOC.")
    
    
# if soh_upload is not None:
#     soh_data = pd.read_excel(soh_upload)
#     predicted_soh = SOC_pred(soc_data)

#     # Display the Pandas DataFrame of SOC data
#     st.write("#### Input SOH Data DataFrame")
#     st.dataframe(soc_data)

#     # Display the predicted SOC
#     st.write("Predicted SOH:")
#     st.write("Predicted SOH ", "{:.2f}".format(predicted_soc[0][0]))  
...
# st.title("GRU Prediction of SOC & SOH")
# st.write("Try uploading Excel files for predicting SOC and SOH.")
...
# # Create file uploaders for SOC and SOH
# soc_upload = sidebar.file_uploader("Upload an Excel File for SOC", type=["xlsx"])
# soh_upload = sidebar.file_uploader("Upload an Excel File for SOH", type=["xlsx"])
...
# # If a file is uploaded, read it into a Pandas DataFrame and perform SOH prediction
# if soh_upload is not None:
#     soh_data = pd.read_excel(soh_upload)
...
#     st.write("#### Input SOH Data DataFrame")
#     st.dataframe(soh_data)

#     # Display the predicted SOH
#     st.write("Predicted SOH:")
#     st.write("Predicted SOH ", "{:.2f}".format(predicted_soh[0][0]))
...
# # Set the page configuration
# st.set_page_config(layout="centered", page_title="SOC & SOH Prediction")
...
#         menu_title=None,
#         options= ["SOC Prediction","SOH Prediction" ,"Project Info"],
...
# if selected == "SOC Prediction":
#     # Read the SOC data from the uploaded file
#     soc_upload = st.file_uploader("Upload an Excel File for SOC", type=["xlsx"])
#     if soc_upload is not None:
#         soc_data = pd.read_excel(soc_upload)
